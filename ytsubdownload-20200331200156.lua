...
function activate()
	...
	_sep=package.config:sub(1,1) --Get path separator
	tmpfolder=getTempPath()

	conffile_path=vlc.config.userdatadir()..string.gsub("/lua/extensions/userdata/ytsubdownload.conf", "/", _sep) --Replace slash with system's separator

	vlc.msg.dbg(conffile_path)
...
function unescape(str) --Parsing Google's double escaped HTML ASCII symbols
  str = string.gsub( str, '&amp;lt;', '<' )
  str = string.gsub( str, '&amp;gt;', '>' )
  str = string.gsub( str, '&amp;quot;', '"' )
  str = string.gsub( str, '&amp;apos;', "'" )
...
function get(host,path)
  ...
  if status == 200 then 
    ...
    vlc.msg.err("[YTSubDownload] HTTP "..tostring(status).." : "..response)
    return false
  end
end
